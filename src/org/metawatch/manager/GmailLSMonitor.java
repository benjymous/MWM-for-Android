...
public class GmailLSMonitor implements GmailMonitor {
	...
	public void startMonitor() {
		for (String account : accounts) {
			try {
				Uri uri = Uri.parse("content://gmail-ls/conversations/"
						+ account);
	...
	private class MyContentObserver extends ContentObserver {
		...
		public void onChange(boolean selfChange) {
			...
			if (Preferences.logging)
				Log.d("ow",
						"gmail observer onChange() : current gmail unread count: "
								+ Integer.toString(currentUnreadCount));

			if (Preferences.notifyGmail && currentUnreadCount > lastUnreadCount) {
				if (Preferences.logging)
					Log.d("ow", Integer.toString(currentUnreadCount) + " > "
							+ Integer.toString(lastUnreadCount));
				sendUnreadGmail();
			}
		...
		for (String account : accounts) {
			try {
				...
				Cursor c = context.getContentResolver().query(
						Uri.parse("content://gmail-ls/labels/" + account),
						null, null, null, null);
				c.moveToFirst();
				...
				while (true) {
					if (c.getString(nameColumn).equals(label)) {
						intUnread += c
								.getInt(c
										.getColumnIndexOrThrow("numUnreadConversations"));
					}
				...
				if (Preferences.logging)
					Log.d(MetaWatch.TAG,
							"GmailLSMonitor.getUnreadCount(): caught exception: "
									+ x.toString());
			}
		}
		// if (Preferences.logging)
		...
		for (String account : accounts) {
			try {
				...
				Cursor c2 = context.getContentResolver()
						.query(Uri.parse("content://gmail-ls/conversations/"
				...
				while (true) {
					if (c2.getString(nameColumn).indexOf(id) >= 0)
						convId = c2.getString(c2
								.getColumnIndexOrThrow("conversation_id"));
				...
				Cursor c3 = context.getContentResolver().query(
						Uri.parse("content://gmail-ls/conversations/" + account
								+ "/" + convId + "/messages"), null, null,
						null, null);
				...
				while (true) {
					if (c3.getString(colConvId).indexOf(convId) >= 0) {
						double thisDate = Double.parseDouble(c3
								.getString(colRcv));

						if (thisDate > maxDate) {
							subject = c3.getString(colSub);
							sender = c3.getString(colFrom);
							snippet = c3.getString(c3
				...
				String senderMail = matcher.group(2).replace("<", "")
						.replace(">", "");

				NotificationBuilder.createGmail(context, senderName,
						senderMail, subject, snippet);
